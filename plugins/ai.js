...
smd(
  ...
  async (m) => {
    try {
      ...
      if (!query) {
        return await m.send("Please provide a query you idiot, e.g., `.gpt What is the weather today?`.");
      }
  ...
  try {
    ...
    if (contentType && contentType.startsWith('image')) {
      ...
      await m.bot.sendFromUrl(
        m.from,
        photoUrl,
        "*PHOTO GENERATED BY Queen_Kylie*:",
        m,
        {},
        "image"
      );
...
smd(
  ...
  async (m) => {
    try {
      ...
      const query = m.text.split(' ').slice(1).join(' ');
      if (!query) {
        return await m.send("Please provide a query you idiot, e.g., `.lumine What is the weather today?`.");
      }
...
smd(
  ...
  async (m) => {
    try {
      ...
      const query = m.text.split(' ').slice(1).join(' ');
      if (!query) {
        return await m.send("Please provide a query dumbo, e.g., `.davinci What is the meaning of life?`.");
      }
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*QUEEN_KYLIE • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
